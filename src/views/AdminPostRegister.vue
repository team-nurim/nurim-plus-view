  ...
  data () {
    return {
      postData: {
        ...
        adminId: 1 // 여기에 adminId를 하드코딩하여 설정합니다.
      },
  ...
  methods: {
    async savePost() {
      try {
        ...
        // API 호출
        const response = await axios.post('http://localhost:8080/api/v1/posts/post/register', formData, {
          headers: {
...
<style>
  ...
  /* 파일 업로드 요소 아래에 추가 정보를 보여주는 스타일링 */
  small {
    display: block;
    margin-top: 5px;
    color: #6c757d;
